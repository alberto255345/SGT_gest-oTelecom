  ...
  <div class="form-group">
    ...
    <table>
      ...
      <tr>
        ...
        <option <?=($FABopc == 'TAIT')?'selected':''?> >TAIT</option>
        <option <?=($FABopc == 'V2COM')?'selected':''?> >V2COM</option>
        <option <?=($FABopc == 'ZIV')?'selected':''?> >ZIV</option>
        <option <?=($FABopc == 'HUGHES')? 'selected':''?> >HUGHES</option>
      </select></td>
      </tr>
      <tr>
...
echo ">";
  ...
  <input type="hidden" id="setorid" name="setorid" <?PHP
if(isset($_GET["setor"])){
  echo 'value="' . $_GET["setor"] . '"'; }
